...
class CreateUserController: Endpoints() {
    ...
    fun createNewUser(username: String, email: String, password: String): Call<CreateResponse> {
        return createUser.postCreateUser(body = CreateRequest(username = username, email = email, password = password))
    }
}